...
def draw_results(img, yolo):
    ...
    for i in range(len(results)):
        ...
        w = int(results[i][3])//2
        h = int(results[i][4])//2
        label=results[i][0]+':'+str(np.round(results[i][5]*100,2))
        cv2.rectangle(img_cp,(x-w,y-h),(x+w,y+h),(0,0,255),3)
        cv2.putText(img_cp,label,(x-w,y-h),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,0),2)
